...
public class CODCredential {
	 ...
	 WebDriver driver;
	    ...
	    // ── MAIN VALIDATION ──────────────────────────────────────────────────
	    public void validateCODAndPlaceOrder(int amount, ExcelUtils excel, int row) throws IOException, InterruptedException  {
	        ...
	        if (isPlaceOrderDisabled()) {
	            ...
	            if (expectedCOD) {
	                ...
	                excel.setCellValue(row, 8, "— COD should NOT be available.");
	    	        excel.setCellColor(row, 8, IndexedColors.RED.getIndex());
	    	        excel.flush();
	            }
	        }
